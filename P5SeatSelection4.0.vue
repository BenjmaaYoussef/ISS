<template>
  <v-app :style="{ background: '#0A0E14' }">
    ...
    <v-app-bar
      flat
      :height="58"
      style="background: rgba(17, 23, 32, 0.97); backdrop-filter: blur(12px); border-bottom: 1px solid rgba(212, 175, 55, 0.18);"
    >
      <v-container fluid class="d-flex align-center px-6">
        ...
        <v-btn
          ...
          size="small"
          style="background: #D4AF37; color: #0A0E14; font-weight: 600; letter-spacing: 0.02em;"
        >
          My Profile
        </v-btn>
        <v-btn
          ...
          size="small"
          style="border: 1px solid rgba(212, 175, 55, 0.35); color: #D4AF37; font-weight: 500;"
        >
          Log Out
        </v-btn>
      </v-container>
    ...
    <div class="filters-bar">
      <v-container fluid class="d-flex align-center px-6 py-2" style="flex-wrap: wrap; gap: 18px;">
        ...
        <div class="filter-group">
          ...
          <v-menu v-model="dateMenu" :close-on-content-click="false">
            <template v-slot:activator="{ props }">
              ...
              >
                {{ selectedDate }}
                <v-icon end size="10" style="color: #D4AF37;">mdi-chevron-down</v-icon>
              </v-btn>
            </template>
            <v-date-picker v-model="selectedDate" @update:model-value="dateMenu = false"></v-date-picker>
          </v-menu>
        </div>
          ...
          <v-menu v-model="timeMenu" :close-on-content-click="false">
            ...
            <v-list style="background: #1A2332;">
              <v-list-item
                ...
                :key="time"
                @click="selectedTime = time; timeMenu = false"
                style="color: #F0EAD6;"
              >
                {{ time }}
              </v-list-item>
    ...
    <v-main>
      <div class="floor-wrap" :style="{ paddingBottom: (selectedTable || selectedSunbed) ? '100px' : '20px' }">
        <div class="floor-plan">
          ...
          <div class="sunbeds-strip">
            ...
            >
              <svg viewBox="0 0 38 20" fill="none" width="38" height="20">
                <rect x="2" y="3" width="10" height="14" rx="2" stroke="currentColor" stroke-width="1.5"/>
                <rect x="14" y="6" width="22" height="9" rx="2" stroke="currentColor" stroke-width="1.5"/>
                <line x1="16" y1="15" x2="16" y2="19" stroke="currentColor" stroke-width="1.3"/>
                <line x1="34" y1="15" x2="34" y2="19" stroke="currentColor" stroke-width="1.3"/>
              </svg>
              <span class="sunbed-num">{{ n }}</span>
              <div class="sb-dot" :class="n === 3 ? 'sb-dot--rose' : 'sb-dot--green'"></div>
            </div>
          </div>
          ...
          <div class="zone-label zone-label--vip" style="top: 392px; left: 200px;">✦ VIP</div>

          <!-- Horizontal zone dividers -->
          <div class="zone-divider" style="top: 224px; left: 130px; width: 690px;"></div>
          <div class="zone-divider zone-divider--vip" style="top: 384px; left: 130px; width: 690px;"></div>
          ...
          >
            <!-- Top chairs -->
            <div class="chair-row">
              <div v-for="i in t.topSeats" :key="'t'+i" class="chair chair--h"></div>
            </div>

            <!-- Middle: left chairs + surface + right chairs -->
            <div class="tu-middle">
              <div class="chair-col">
                <div v-for="i in t.leftSeats" :key="'l'+i" class="chair chair--v"></div>
              </div>
              <div class="tu-surface" :class="t.vip ? 'tu-surface--round' : ''"></div>
              <div class="chair-col">
                <div v-for="i in t.rightSeats" :key="'r'+i" class="chair chair--v"></div>
              </div>
            </div>

            <!-- Bottom chairs -->
            <div class="chair-row">
              <div v-for="i in t.bottomSeats" :key="'b'+i" class="chair chair--h"></div>
            </div>
    ...
    <div class="action-bar" :class="{ 'action-bar--visible': selectedTable !== null || selectedSunbed !== null }">
      <v-container fluid class="d-flex align-center justify-space-between px-6">
        ...
        <div class="selection-summary" v-if="selectedTable !== null">
          ...
          <div class="summary-info">
            ...
            <span class="summary-meta">{{ getSelectedInfo()?.seats }} seats · {{ selectedDate }} · {{ selectedTime }}</span>
          </div>
        </div>
...
const timeOptions = [
  '17:00h', '17:30h', '18:00h', '18:30h', '19:00h',
...
const allTables = ref([
  // Zone A
  { id:1, name:'Table A1', seats:4, status:'available', vip:false,
    x:200, y:82, topSeats:2, bottomSeats:2, leftSeats:0, rightSeats:0 },
  { id:2, name:'Table A2', seats:4, status:'reserved', vip:false,
    x:380, y:82, topSeats:2, bottomSeats:2, leftSeats:0, rightSeats:0 },
  { id:3, name:'Table A3', seats:2, status:'available', vip:false,
    x:560, y:82, topSeats:1, bottomSeats:1, leftSeats:0, rightSeats:0 },
  { id:4, name:'Table A4', seats:2, status:'available', vip:false,
    x:680, y:82, topSeats:1, bottomSeats:1, leftSeats:0, rightSeats:0 },

  // Zone B
  { id:5, name:'Table B1', seats:6, status:'available', vip:false,
    x:200, y:250, topSeats:3, bottomSeats:3, leftSeats:0, rightSeats:0 },
  { id:6, name:'Table B2', seats:4, status:'available', vip:false,
    x:400, y:250, topSeats:2, bottomSeats:2, leftSeats:0, rightSeats:0 },
  { id:7, name:'Table B3', seats:6, status:'reserved', vip:false,
    x:575, y:250, topSeats:2, bottomSeats:2, leftSeats:1, rightSeats:1 },

  // Zone VIP
  { id:8, name:'VIP 1', seats:8, status:'available', vip:true,
    x:200, y:405, topSeats:2, bottomSeats:2, leftSeats:2, rightSeats:2 },
  { id:9, name:'VIP 2', seats:8, status:'reserved', vip:true,
    x:420, y:405, topSeats:2, bottomSeats:2, leftSeats:2, rightSeats:2 },
  { id:10, name:'VIP 3', seats:6, status:'available', vip:true,
    x:640, y:405, topSeats:2, bottomSeats:2, leftSeats:1, rightSeats:1 },
])
...
const getSelectedInfo = () => allTables.value.find(t => t.id === selectedTable.value)
...
const decrementGuests = () => { if (guestCount.value > 1) guestCount.value-- }
</script>
...
.wall--top {
  ...
  right: 0;
  height: 42px;
  background: linear-gradient(90deg, rgba(14, 60, 120, 0.55), rgba(0, 100, 180, 0.28));
  color: #7EC8E3;
