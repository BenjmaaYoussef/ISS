  ...
  <v-dialog :model-value="modelValue" @update:model-value="$emit('update:modelValue', $event)" max-width="520" persistent>
    <v-card style="background: #13181f; border: 1px solid rgba(212,175,55,0.18);">
      <v-card-title style="font-family: var(--font-heading); color: #fff; padding: 20px 20px 12px;">
        <v-icon start color="#D4AF37" class="mr-2">mdi-walk</v-icon>
        Seat a Walk-in
      </v-card-title>
      ...
      <v-card-text style="overflow-y: auto; max-height: 60vh; padding: 0 20px 8px;">
        ...
        <div v-if="allBlocked" class="walkin-empty">
          <v-icon size="36" color="rgba(255,255,255,0.15)">mdi-table-off</v-icon>
          <span>No tables available for walk-in right now.</span>
        </div>

        <!-- Table list -->
        <v-list v-else density="compact" class="walkin-list pa-0">
        ...
        <template v-if="selectedTable">
          ...
          <div class="guest-section mb-3">
            <div class="guest-section-label mb-2">
              <v-icon size="13" class="mr-1" style="color: #D4AF37;">mdi-account-search-outline</v-icon>
              Guest account
            ...
            <div v-if="selectedUser" class="selected-user-chip">
              <v-icon size="14" color="#D4AF37" class="mr-1">mdi-account-circle-outline</v-icon>
            ...
            <div v-else class="user-search-wrap">
              <v-icon size="14" class="us-icon">mdi-magnify</v-icon>
              <input
                v-model="userQuery"
                class="us-input"
                ...
                @blur="onSearchBlur"
              />
              <button v-if="userQuery" class="us-clear" @mousedown.prevent @click="userQuery = ''; userResults = []">
                <v-icon size="13">mdi-close</v-icon>
              </button>

              <!-- Results dropdown -->
              <div v-if="userResults.length" class="us-dropdown">
                <button
                  ...
                  @click="selectUser(u)"
                >
                  <v-icon size="14" color="rgba(255,255,255,0.3)" class="mr-2">mdi-account-outline</v-icon>
      ...
      <v-card-actions style="padding: 12px 20px 16px; border-top: 1px solid rgba(255,255,255,0.06);">
        <v-spacer />
        <v-btn variant="text" color="rgba(255,255,255,0.5)" @click="cancel">Cancel</v-btn>
        <v-btn
          color="#D4AF37"
          variant="tonal"
          :disabled="!canConfirm"
...
watch(selectedTable, () => { partySize.value = 1 })
...
function onSearchBlur() {
  ...
  setTimeout(() => { userResults.value = [] }, 150)
}
...
function parseTime(t) {
  ...
  if (m24) return parseInt(m24[1], 10) * 3600000 + parseInt(m24[2], 10) * 60000
  return 0
}
...
function buildConflictMap() {
  ...
  for (const table of props.freeTables) {
    ...
    if (next) {
      minutesUntilNext = Math.round((parseTime(next.time) - nowMs) / 60000)
